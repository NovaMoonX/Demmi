...
- **Modular Action Registry**: The chat engine uses a generic action registry (`getActionHandler`) to dispatch AI responses — `Chat.tsx` contains no action-specific logic, making it trivially easy to add new AI actions without touching the chat component. Each action handler encapsulates its own streaming, parsing, and state updates.
