...
- **Generic Store Selector Runtime Bridge**: Action handlers receive a generic `reduxSelector` runtime callback so any action step can read Redux state via selectors when needed (for example, meal generation checks existing ingredients) without importing the store directly in action modules.
